...
t <- readr::read_csv("responses.csv")
names(t)
...
average_confidences <- dplyr::group_by(t_tidy, question) |>
...
confidences_on_taught_sessions <- t_tidy |>
  dplyr::filter(!is.na(answer)) |>
  dplyr::filter(question %in% t_sessions_taught)
